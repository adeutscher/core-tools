...
countdown_to ()
{
    ...
    local _th="$(date -d "${_t}")";
    if [ "${_ds}" -gt "${_ts}" ]; then
        error "$(printf "Requested date is ${BOLD}%s${NC} in the past: ${BOLD}%s${NC}" "$(__translate_seconds "$((${_ds}-${_ts}))")" "$(date -d "${_t}")")";
        return 1;
    fi;
    notice "$(printf "Counting down to time: ${BOLD}%s${NC}" "${_th}")";
    countdown_seconds "$((${_ts}-${_ds}))"
}
