...
check_hak5(){
  ...
  for __prefix in $__prefixes; do
    ...
    done <<< "$(find /sys/class/net -maxdepth 1 -mindepth 1)"

    # Look for ARP entries matching a prefix.
    while read -r __arp_match; do
...
check_key_logins(){
    ...
    local __key_file
    __key_file="$(grep -Pm1 "^\s*[^#]*\s*AuthorizedKeysFile" "/etc/ssh/sshd_config" 2> /dev/null | awk -F' ' '{ print $2}')"
    if [ -z "$__key_file" ]; then
        # If we cannot read SSH config for whatever reason, then assume default.
        __key_file=".ssh/authorized_keys"
    fi
    notice "$(printf "Checking for users with SSH keyfiles at ${GREEN}~/%s${NC}" "$__key_file")."

    while read -r __line; do
        local __user
        __user="$(cut -d':' -f1 <<< "$__line")"
        local __home
        __home="$(cut -d':' -f6 <<< "$__line")"
        local __uid
        __uid="$(cut -d':' -f3 <<< "$__line")"
    ...
    if (( __home_total )); then
        ...
        notice "$(printf "Found absolutely no ${GREEN}%s${NC}-based local users in ${GREEN}%s${NC}." "/home" "/etc/passwd")"
    fi
    ...
    if (( __non_home_total )); then
        ...
        error "$(printf "Found absolutely no non-${GREEN}%s${NC}-based users." "/home")"
    fi
...
check_masked_processes(){
    ...
    while read -r __proc; do
        if [ -z "$__proc" ] || ! [ -d "$__proc" ]; then
            continue
        fi

        # Get information
        local __exe
        __exe="$(readlink "$__proc/exe")"
        local __cmd
        __cmd="$(tr '\0' ' ' < "$__proc/cmdline" | cut -d' ' -f1)"
        ...
        local __exe_clean
        __exe_clean="$(basename "$(sed -r -e 's/\ \(deleted\)$//g' -e 's/;[^\s]{0,1}*//g' <<< "$__exe")")"
        local __cmd_clean
        # shellcheck disable=SC2001
        __cmd_clean="$(basename "$(readlink -f "$(sed -e 's/^\-//g' <<< "$__cmd")")" | sed 's/:$//')"
