...
if qtype xfreerdp; then
    ...
    # NOTE: The regular RDP resolution in the 'rdp' function defaults to a WxH of 1600x900.
    #    If you want to override this on a particular host,
    #      set the RDP_WIDTH and RDP_HEIGHT environment variables.
    if xfreerdp 2> /dev/null | grep -q \/flag; then
        # If xfreerdp has the /flag option,
        #   then  it is assumed to have all of the new-style switches.
        rdp (){
            local server=$1
            shift
