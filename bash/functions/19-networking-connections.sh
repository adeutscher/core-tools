...
if __is_unix; then
    ...
    connections-in-all(){
        ...
        netstat -tun | grep ESTABLISHED  | awk '{ split($4,l,":"); split($5,r,":"); if(l[2] < '$(cat /proc/sys/net/ipv4/ip_local_port_range | awk '{ print $1 }')'){ print r[1] "." l[1] "." l[2] "." $1 "." r[1] "->" l[1] " ("$1"/"l[2] }; }' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n -k 5,5n -k 6,6n -k 7,7n -k 8,8n -k 9,9n -k 10,10 | cut -d'.' -f 11- | uniq -c | awk -F' ' '{ if($1 > 1){ print $2" "$3", "$1" connections)" } else { print $2" "$3")" } }'
    }
    ...
    connections-out-all(){
        ...
        # Making this an alias is a nightmare that I didn't want to have to deal with.
        netstat -tun | grep ESTABLISHED  | awk '{ split($4,l,":"); split($5,r,":"); if(l[2] > '$(cat /proc/sys/net/ipv4/ip_local_port_range | awk '{ print $1 }')' && ($1 == "tcp" || r[2] < '$(cat /proc/sys/net/ipv4/ip_local_port_range | awk '{ print $1 }')')){ print l[1] "." r[1] "." r[2] "." $1 "." l[1] "->" r[1] " ("$1"/"r[2] }; }' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n -k 5,5n -k 6,6n -k 7,7n -k 8,8n -k 9,9n -k 10,10 | cut -d'.' -f 11- | uniq -c | awk -F' ' '{ if($1 > 1){ print $2" "$3", "$1" connections)" } else { print $2" "$3")" } }'
    }
